...
/**
 * Puts a link where a person can paste it: the share sheet where there is one,
 * the clipboard otherwise, and a selectable field when even that is refused.
 * ShareCardButton and the battle result's share control are both this.
 *
 ...
 * shared page, where this sits beside the wordmark rather than among
 * the calls to action.
 */
export default function ShareLinkButton({
  ...
  url: string;
  /** The button's accessible name, e.g. "Share Untitled #4". */
  label: string;
  /** Names the fallback field, e.g. "Card link, copy it manually". */
  fieldLabel: string;
  /** Once per completed share: a sheet that closed on a target, or a copy. */
  onShared: () => void;
  variant: "icon" | "corner";
}) {
  ...
  async function share() {
    ...
    // A crawler may be the first visitor to this route. Start its OBJKT lookup
    // while the person chooses where to share; shareServer's persistent cache
    // then makes both the crawler HTML and OG image reuse the result. This is
    // deliberately fire-and-forget so sharing never waits on warming.

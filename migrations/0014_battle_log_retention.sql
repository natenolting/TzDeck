-- battle_log grows one row per settled battle with no cap today. Nothing in
-- production reads it beyond commit_battle's own 7-day decay lookback (see
-- battle_log_decay_idx) -- there is no user-facing battle-history feature to
-- preserve. Sweep opportunistically inside commit_battle rather than adding
-- separate cron infra, the same pattern 0012 used for rate_limits. 30 days
-- is a comfortable margin over the 7-day decay window, not a tight bound.
CREATE INDEX battle_log_settled_at_idx ON battle_log (settled_at);

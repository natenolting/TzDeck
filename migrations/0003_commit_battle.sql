...
-- exactly two wallets and two progress rows per battle, a simple two-way
-- comparison is a complete, correct implementation of "sorted" -- and
-- because every invocation uses the SAME global ordering rule, this also
-- prevents a deadlock across a 3+-wallet concurrent cycle, not just the
-- 2-wallet reversed-role case, without needing a variable-length lock loop.
CREATE FUNCTION commit_battle(
  ...
  winner_new_xp bigint,
  winner_new_level integer,
  loser_recovery_until timestamptz
) AS $$
DECLARE
  ...
  v_first_progress wallet_card_progress;
  v_second_progress wallet_card_progress;
...
BEGIN
  ...
  IF v_attempt IS NULL OR v_attempt.generation != p_generation OR v_attempt.status != 'pending' THEN
    RETURN QUERY SELECT false, 'attempt_not_claimable'::text, NULL::bigint, NULL::integer, NULL::timestamptz;
    RETURN;
  END IF;
  ...
  BEGIN -- nested block: an expected business rejection rolls back everything below, without aborting the outer call
    ...
    END IF;
    IF p_attacker_expected_version IS NULL AND v_attacker_progress.progress_version != 0 THEN
    ...
    IF p_outcome = 'win' THEN
      ...
      RETURNING xp INTO winner_new_xp;
      ...
      RETURNING recovery_until INTO loser_recovery_until;
    ELSE
    ...
    UPDATE battle_attempts SET status = 'completed', completed_at = v_settlement_time,
      response = jsonb_build_object('outcome', p_outcome, 'xpAwarded', v_xp_award)
    WHERE nonce = p_nonce;

    RETURN QUERY SELECT true, NULL::text, winner_new_xp, NULL::integer, loser_recovery_until;
    RETURN;
  EXCEPTION
    WHEN SQLSTATE 'BT001' OR SQLSTATE 'BT002' OR SQLSTATE 'BT003' OR SQLSTATE 'BT004'
      ...
      OR SQLSTATE 'BT009' OR SQLSTATE 'BT010' THEN
    ...
    UPDATE battle_attempts SET status = 'failed', response = jsonb_build_object('error', SQLERRM), retryable = false
    WHERE nonce = p_nonce AND generation = p_generation;
    RETURN QUERY SELECT false, SQLERRM::text, NULL::bigint, NULL::integer, NULL::timestamptz;
    RETURN;
  END;
END;
$$ LANGUAGE plpgsql;

-- Extends commit_battle's (0005) response with the opponent identity and
-- combat detail the client needs to render a battle-log/result screen. Every
-- one of these values is already computed by the route and passed in as
-- p_inputs (attackerStats, defenderStats, and combat -- including its
-- round-by-round history) or is already a function parameter (defender
-- wallet/card key) -- this only changes what v_result forwards, not any
-- game logic above it.
